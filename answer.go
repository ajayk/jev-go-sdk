...
// Answer is one typed answer: a [NoulAnswer], [ChoiceAnswer], or
// [ScoreAnswer]. Switch on the concrete type, or use the typed getters on
// [Response].
...
// Level returns the index of the most probable rubric level and its
// probability. Ties resolve to the lowest index. It returns -1 and 0 when
// there are no probabilities.
func (a ScoreAnswer) Level() (int, float64) {
...
// produced it. A distribution must cover exactly the declared options or
// rubric levels and sum to one, so a consumer never reads a missing entry as
// zero or a foreign key as a level.
...
// isLevelKey reports whether key is the canonical decimal index of one of n
// rubric levels ("0" .. "n-1").
...
// entries, every value in [0, 1], and a total of one within rounding
// tolerance.
...
func distributionTolerance(n int) float64 { return 0.01 + 0.001*float64(n) }
